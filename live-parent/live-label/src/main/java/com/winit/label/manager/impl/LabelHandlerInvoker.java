...
import com.winit.label.shippment.entity.LabelEntity.StatuType;
...
public class LabelHandlerInvoker 
{
	...
	public ResponseMessage invoke(RequestMessage requestMessage)
	{
		...
		try
		{
			...
			LabelEntity label = labelService.findByDocumentNo(documentNo);
			...
			else //label not exists, create a new one
			{
				...
				label.setTrackingNo(result.getTrackNo());
				label.setStatus(StatuType.Got);
                label.setDeliveryWayId(deliveryWay.getId());
			}
...
//			
	 ...
	 * @return
	 */
	private String getUploadFileName(LabelEntity label,DeliveryWay deliveryWay){
	 ...
	 * @param isNew
	 */
	private void save(LabelEntity label){
		if(label.getId()==null){
